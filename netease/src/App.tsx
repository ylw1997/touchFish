...
function App() {
  ...
  useEffect(() => {
    ...
    void syncAuthState();
    void request("NETEASE_GET_QUALITY" as any, null).catch(console.error);
 
    const handleMessage = (event: MessageEvent) => {
      const payload = event.data;
      if (payload?.command === "NETEASE_AUTH_SYNC") {
        if (payload.payload?.isLoggedIn && payload.payload?.userInfo) {
          login(payload.payload.userInfo);
        } else {
          logout();
        }
      } else if (payload?.command === "NETEASE_QUALITY_SYNC") {
    ...
    <div className="qqmusic-app" style={{ fontSize: `${fontSize}px` }}>
      ...
      <div ref={groupRef}>
        ...
        <FloatButton.Group
          ...
          shape="circle"
          style={{ insetInlineEnd: 24, bottom: currentSong ? 160 : 152 }}
