...
const PlaylistDrawer: React.FC<PlaylistDrawerProps> = ({
  ...
  const { playSong, getPlaylistDetail } = useQQMusic();
  ...
  const loadPlaylistDetail = useCallback(async (pageNum: number = 1) => {
    ...
    try {
      const result = await getPlaylistDetail(playlist.dissid, pageNum, 50);
  ...
  }, [playlist?.dissid, getPlaylistDetail]); // 移除 songs.length 依赖，彻底破开死循环
