...
async function poll(){try{const s=await api('/api/sessions/'+id);statusEl.textContent=s.message;statusEl.className=s.state==='verified'?'ok':(s.state==='failed'||s.state==='expired'?'error':'');meta.textContent=s.cookieReady?'Cookie 已验证：'+s.cookieFieldCount+' 个字段，'+s.cookieLength+' 个字符'+(s.user?.nickname?'，账号：'+s.user.nickname:''):'';if(s.state==='verified'||s.state==='failed'||s.state==='expired'){clearInterval(timer);button.disabled=false;button.textContent='重新生成';if(s.state==='verified')qr.style.display='none'}}catch(e){statusEl.textContent=e.message;statusEl.className='error'}}
button.onclick=async()=>{clearInterval(timer);if(qrUrl)URL.revokeObjectURL(qrUrl);button.disabled=true;statusEl.className='';statusEl.textContent='正在启动浏览器并获取二维码…';meta.textContent='';qr.style.display='none';try{const s=await api('/api/sessions',{method:'POST'});id=s.id;token=s.claimToken;const response=await fetch('/api/sessions/'+id+'/qr',{headers:{'X-Claim-Token':token}});if(!response.ok)throw new Error((await response.json()).error);qrUrl=URL.createObjectURL(await response.blob());qr.src=qrUrl;qr.style.display='block';statusEl.textContent=s.message;button.textContent='等待扫码';timer=setInterval(poll,1200)}catch(e){statusEl.textContent=e.message;statusEl.className='error';button.disabled=false}};
</script></body></html>`;
