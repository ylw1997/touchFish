...
function getCommonHeaders(
  cookie: string,
  referer: string = "https://linux.do/latest",
) {
  return {
    accept:
  ...
  while ((itemMatch = itemRegex.exec(xmlString)) !== null) {
    ...
    const extractField = (fieldName: string) => {
      const pattern = new RegExp(
        `<${fieldName}[^>]*><!\\[CDATA\\[([\\s\\S]*?)\\]\\]><\\/${fieldName}>|<${fieldName}[^>]*>([\\s\\S]*?)<\\/${fieldName}>`,
        "i",
      );
...
export const getNewsList = async (tab: string = "latest") => {
  ...
  try {
    ...
    if (
      ...
      error.message?.includes("touchfish.linuxDoCookie")
    ) {
      return {
        data: [
          {
            title: "LinuxDo Cookie 未配置/失效 (点击配置)",
            url: "configure_linuxdo_cookie",
            time: new Date().toISOString(),
            author: "System",
            category: "System",
            description: "请点击配置 LinuxDo Cookie",
            guid: "configure_linuxdo_cookie",
          },
        ],
      };
    }
...
export const getNewsDetail = async (url: string) => {
  ...
  try {
    ...
    if (typeof response.data !== "string" || !response.data.includes("<?xml")) {
      throw new Error("返回数据格式错误，可能是 Cookie 无效");
    }
