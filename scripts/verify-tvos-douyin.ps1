...
$checks = @(
    ...
    @{ Path = 'ios/TouchFishTV/TouchFishTV/PlaybackCoordinator.swift'; Pattern = 'func\s+isOwned\(by\s+owner:\s*PlaybackOwner\)'; Description = 'global playback event ownership check' },
    ...
    @{ Path = 'ios/TouchFishTV/TouchFishTV/Views/FavoritesLibraryView.swift'; Pattern = 'private\s+var\s+generation:\s*UInt'; Description = 'favorite request generation' },
    @{ Path = 'ios/TouchFishTV/TouchFishTV/Views/FavoritesLibraryView.swift'; Pattern = 'selectedIndex\s*==\s*expectedIndex[\s\S]*?playbackToken\s*==\s*expectedToken'; Description = 'favorite async playback selection guard' },
